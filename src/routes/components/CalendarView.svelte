...
<div class="CalendarGrid">
	<div class="month-menu-container">
		...
		<div id="month-menu-month" on:click|capture={clickBlankArea}>
			{monthList[chosenDate.month]}
			{chosenDate.year}
		</div>
	...
	{#each Array(numberOfBlanks) as _, index (index)}
		<div class="date blank" on:click|capture={clickBlankArea} />
	{/each}

	{#each Array(daysInCurrentMonth) as _, index (index)}
		<div
			class="date"
			class:todayDate={index + 1 == dateInfo.day && chosenDate.month == dateInfo.month}
			on:click|capture={() => clickNewTodo(index + 1)}
		>
			{index + 1}

			{#each chosenMonthTodoList as todo (todo.id)}
				{#if todo.dueDate.slice(8, 10) == index + 1}
					<div class="calendar-todo {todo.colourCode}" on:click={() => clickTodo(todo.id)}>
						{todo.todoName}
