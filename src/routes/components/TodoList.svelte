			...
			{#each filteredTodoList as todo (todo.id)}
				...
				{:else}
					<li>
						<div
							class="todo-header"
							class:selected={selectedTodo === todo.id}
							on:click={() => clickTodo(todo.id)}
						>
							<div class="todo-name">{todo.todoName}</div>
							<div class={todo.dateFlag}>{todo.prettyDate}</div>
							<form method="POST" action="../apis/todo?/removeTodoFromDB" use:enhance>
								<button class="del-button" type="submit" on:click|stopPropagation={stopProp}
									>✘</button
								>
						...
						{#if selectedTodo === todo.id}
							<div class="todo-data">
								...
								<button class="edit-button" type="submit" on:click={() => (editTodoFlag = todo.id)}
									>E</button
								>
							</div>
						{/if}
